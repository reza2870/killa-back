...
export class UnstakeTxn extends TxnBatch {
	type = 'unstake';
	async _build() {
		const tokens = this.request.tokens;

		const rows = await db.query(`SELECT * FROM sol_token WHERE eth_token_id IN (${db.escape(tokens)}) AND staking_pending = 0 AND staked = 1`);
		...
		for (let i = 0; i < tokens.length; i++) {
			...
			let txn = await buildUnstake(this.umi, false, addresses, blockhash);
	...
	async afterSubmit(results) {
		...
		for (let i = 0; i < this.request.tokens.length; i++) {
			...
			let q = `UPDATE sol_staking_event SET ${querify({ status })} WHERE batch_id = ${db.escape(this.id)} AND eth_token_id = ${db.escape(token)} AND action = 'unstake'`;
			queries.push(q);
			if (status == 'error') {
